#include "PmxReading.h"

#include <d3dcompiler.h>
#include <filesystem>

PmxData PmxReading::PmxRead(const char* fileName)
{
 ...
 * @param fp ファイルデータ
 */
void PmxReading::Header(FILE* fp)
{
	// ヘッダーチェック
	BYTE header[4];
	fread(header, 4, 1, fp);
 ...
 * @param fp ファイルデータ
 */
void PmxReading::Vertex(FILE* fp)
{
	// 頂点数を読み込む
 ...
 * @param fp ファイルデータ
 */
void PmxReading::Surface(FILE* fp)
{
	//面
 ...
 * @param fp ファイルデータ
 */
void PmxReading::Texture(FILE* fp)
{
	//テクスチャ
 ...
 * @param fp ファイルデータ
 */
void PmxReading::Material(FILE* fp)
{
	//マテリアル
...
/**
	...
	@return 成功　失敗
*/
bool PmxReading::GetPMXStringUTF16(FILE* file, std::wstring& output)
{
	if (file == NULL)
		return false;
